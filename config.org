...
** GraphQL
   ...
   [[https://github.com/davazp/graphql-mode][~graphql-mode~]] allows editing GraphQL schema and queries.
   ...
   Alternative: [[https://github.com/jdormit/ob-graphql][jdormit/ob-graphql: GraphQL execution backend for org-babel]]
...
*** Org-cliplink
    ...
    I couldn’t think of good keybinding for this.
...
*** Searching
    ...
    Org-roam [[https://www.orgroam.com/manual/Full_002dtext-search-interface-with-Deft.html#Full_002dtext-search-interface-with-Deft][documentation recommends Deft]], but in the author's Emacs config I
    found out about [[https://github.com/Wilfred/deadgrep][deadgrep]], which works with [[https://github.com/BurntSushi/ripgrep][ripgrep (rg)]] — a grep
    alternative.

    It's optimized for code searching but I think it can work well for Org-roam
    directory.
...
* Font
  ...
  #+BEGIN_SRC emacs-lisp
  ; None of these work for frames created with emacsclient -c
  ;(set-frame-font "Monoid-12" nil t)
  ;(set-default-font "Monoid-12")
  ;(set-face-attribute 'default nil :font "Monoid" :height 120)

  ;; Finally found a solution: https://stackoverflow.com/a/41355013/1787829
...
[[https://github.com/hlissner/doom-emacs/blob/e65e2e/modules/config/default/%2Bemacs-bindings.el#L359][~doom-emacs~ uses]] ~"C-c v g"~ or ~"SPC v g"~ (vim style).
[[https://github.com/ergoemacs/ergoemacs-mode/blob/f9d6e3/ergoemacs-themes.el#L918][~ergoemacs-mode~ uses]] ~<apps> n m~ with ~<apps>~ being [[http://ergoemacs.org/emacs/emacs_menu_app_keys.html][the Menu key]].
[[https://develop.spacemacs.org/layers/+source-control/git/README.html#working-with-git][Spacemacs uses]] ~M-m g s~ or ~SPC g s~.
...
Integration between Magit and [[https://github.com/RichiH/vcsh][vcsh]].

#+begin_src emacs-lisp
(use-package magit-vcsh
  :after vcsh
  :ensure t
  :bind (("M-g h" . magit-vcsh-status)))
...
*** Hunspell
   ...
   Install spellchecker utility and dictionaries:

   #+BEGIN_SRC fish
   sudo apt install -y hunspell hunspell-en-us hunspell-pt-br
   #+END_SRC

   #+begin_quote
   Hunspell is a free spell checker and used by LibreOffice, Firefox and Chromium. It allows to set multiple dictionaries - even different dictionaries per language (aspell, for example also allows multiple dictionaries, but only for the same language). — [[https://200ok.ch/posts/2020-08-22_setting_up_spell_checking_with_multiple_dictionaries.html][Setting up spell checking with multiple dictionaries in Emacs - 200ok]]
   #+end_quote

   Set up Ispell to use Hunspell:
   ...
   (use-package ispell
     ...
     (setq
      ...
      ispell-dictionary "en_US,pt_BR"            ; Default dictionnary
     ...
     (setq ispell-personal-dictionary "~/Mega/Data1/Apps/hunspell_personal.dic")
   ...
   If you get an error saying that =ispell-hunspell-add-multi-dic= is void, run =list-load-path-shadows= to check if an older version of =ispell.el= is not shadowing a newer installation.

   I had to delete the files =flyspell.el= and =ispell.el= from =/usr/share/emacs/site-lisp/dictionaries-common=. They were probably leftovers from a previous Emacs version I uninstalled. I found the answer on [[https://emacs.stackexchange.com/questions/21378/spell-check-with-multiple-dictionaries#comment36562_21379][StackExchange]].

*** Issues with Hunspell

    I’ve noticed Hunspell 1.6.2 doesn’t work correctly for words with apostrophe. A lot of them are shown as misspelled when they are in fact correct.

   Not working: didn't, didn’t, I’ve, I've.

   Working: 'apple', ‘apple’, someone's, someone’s, can't, can’t.

   More at [[https://github.com/en-wl/wordlist/issues/122][en-wl/wordlist#122 hunspell can't handle words with an apostrophe...]]

   After reading [[https://battlepenguin.com/tech/aspell-and-hunspell-a-tale-of-two-spell-checkers/][Aspell and Hunspell: A Tale of Two Spell Checkers]] I also discovered some words which Hunspell offers incorrect spellings:

   - excersized :: (0) supersized
   - enterperuner :: (0) interpenetrate (1) serpentine
   - presuing :: (0) presuming (1) pressing (2) pressuring (3) presupposing (4) pressurizing (5) presaging
   - discoved :: (0) discoed (1) discover (2) discommoded

   Aspell provides correct spellings:

   - excersized :: exercised, excised, excesses, exercises, exorcised, exercise's, exercisers, exerciser's
   - enterperuner :: entrepreneur, Enterprise, enterprise, interpreter, interferon
   - presuing :: pressing, perusing, presuming, presiding, pressuring, praising, reusing, pursuing, presaging, pressings, pressing's
   - discoved :: discoed, discover, discovers, discord, discovered, discorded, discovery, dissolved

   #+begin_quote
   So why is Hunspell used everywhere, even though Aspell seems to have superior correction capabilities? One of the issues is that Aspell is no longer maintained. It’s last release was in 20113, where as Hunspell is actively maintained and used in LibreOffice, Firefox, Thunderbird and Chrome. — [[https://battlepenguin.com/tech/aspell-and-hunspell-a-tale-of-two-spell-checkers/][Aspell and Hunspell: A Tale of Two Spell Checkers]]
   #+end_quote

   Besides, using multiple dictionaries with Aspell seems to be a lot harder:

   - [[https://unix.stackexchange.com/questions/341714/][Use multi language dictionary with aspell - Unix & Linux Stack Exchange]]
   - [[https://emacs.stackexchange.com/questions/27027][How to supply ispell program with dictionaries? - Emacs Stack Exchange]]
   - [[https://stackoverflow.com/questions/50816864/][linux - adding many dictionaries to aspell - Stack Overflow]]
